(function() {
  ...
  COMMANDS = ['new', 'init', 'serve', 'build'];

  SWITCHES = [
    ['-o', '--open [Config Path]', 'argument: [Config Path] | watch files for changes and run for develop'],
    ['-a', '--assets [Config Path]', 'argument: [Config Path] | run command without entry html'],
    ...
    switch(true){
      ...
      case !!opts.build:
        ...
        if(arg && arg.indexOf('.json') > -1){
          ...
          configPath = arg ? path.join(process.cwd(), arg) : process.cwd();
          process.env.CONFIG_FILE = path.join(configPath, 'config.build.json');
        }
        handler('build');
        break;
      case !!opts.dll:
      ...
      default:
        ...
        if(arg && arg.indexOf('.json') > -1){
          ...
          configPath = arg ? path.join(process.cwd(), arg) : process.cwd();
          process.env.CONFIG_FILE = path.join(configPath, 'config.dev.json');
        }
        handler('dev');
        break;
    }
  ...
  boilerplate = function(dirname){
    ...
    var cmdStr = 'cp -avX ' + path.join(__dirname, '../boilerplate') + ' ' + path.join(process.cwd(), dirname);
    exec(cmdStr, function(err, stdout, stderr){
      if(err) {
        console.log(stderr);
      } else {
        console.log('new project [' + dirname + '] had created success!');
      }
    });
  }
  ...
  init = function(){
    var exec = require('child_process').exec; 
    var cmdStr = 'cp -avX ' + path.join(__dirname, '../boilerplate/*') + ' ' + path.join(process.cwd());
    exec(cmdStr, function(err, stdout, stderr){
      if(err) {
        console.log(stderr);
      } else {
