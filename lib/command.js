(function () {
  ...
  // \n\nIf called without options, cli will run `damo serve --open` in current
  // directory.
  ...
  parseOptions = function () {
    ...
    if (isOption && COMMANDS.indexOf(args[0]) > -1) {
      ...
      if (rule = rulesMap[args[0]]) {
        options[rule.name] = rule.hasArgument
          ? args[1]
          : true;
        options.arguments = args
          .slice(1)
  ...
  exports.run = function () {
    ...
    switch (true) {
      ...
      case !!opts.build:
        ...
        if(arg === 'assets'){
          process.env.ASSETS = true;
          opts['arguments'].splice(0, 1);
        }
      ...
      default:
        ...
        var idx = opts['arguments'].findIndex(a => a === 'hot');
        if (idx > -1) {
          process.env.HOT = true;
          opts['arguments'].splice(idx, 1);

          if (fs.existsSync(path.join(process.cwd(), 'node_modules/webpack')) && fs.existsSync(path.join(process.cwd(), 'node_modules/webpack'))) {
            run();
          } else {
            printLine('make sure you had installed webpack and webpack-dev-server，waiting for a few min' +
                'utes...');
            var exec = require('child_process').exec;
            var cmdStr = 'npm install webpack@1.13.3 --registry=http://r.cnpmjs.org && npm install webpack' +
                '-dev-server@1.14.1  --registry=http://r.cnpmjs.org';
